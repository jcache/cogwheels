 ...
 * @return {LibSass.ImportSource}
 */
Importer.prototype.find = function Importer$find(uri, prev) {
  var search  = new Search({ graph: this.graph, uri: uri, source: prev })
    , paths   = this.allPossiblePathsFor(uri, prev)
  ;

  return search.execute(paths);
};
...
Importer.prototype.allPossiblePathsFor = function Importer$allPossiblePathsFor(uri, prev) {
  let resolved_uri  = path.resolve(path.dirname(prev), uri)
    , matching_uri  = this.matching_uri_for(resolved_uri)
  ;

  let potentially_matching_uris = [matching_uri];
...
Importer.prototype.possible_paths_for = function Importer$possible_paths_for(uri) {
  let base_dir  = path.dirname(uri)
    , basename  = path.basename(uri)
  ;

  let join_format = (format) => path.join(base_dir, util.format(format, basename));

  let possible_uris = FILENAME_FORMATS.map(join_format);
...
Importer.prototype.matching_uri_for = function Importer$matching_uri_for(resolved_uri) {
  let matching_root = _.detect(this.paths, (p) => resolved_uri.indexOf(p) === 0);
...
function generate_full_paths_for(paths, possible_uris) {
  let combine_paths = (load_path) => possible_uris.map(
    (possible_uri) => path.join(load_path, possible_uri)
  );

  return _.chain(paths).map(combine_paths).flatten().value();
}
